...
import { getpatientprofiledetail, PatientHistoryData, GetDiagnosticReport, DiscardRecord } from '../../api/api';
import Icon from 'react-native-vector-icons/MaterialIcons';
import Pdf from 'react-native-pdf';

const GetSaveDiagnostic = ({ patientId }) => {
  ...
  useEffect(() => {
    const fetchPatientHistory = async () => {
      ...
      try {
        ...
        const res = await PatientHistoryData({ uhid: response.data.uhid, hiType: 1 });
        setHistory(res.data);
      } catch (err) {
  ...
  const handleCardPress = async (docId) => {
    try {
      ...
      const res = await GetDiagnosticReport({ docid: docId });
      if (res.data) {
        const source = {
      ...
      console.error('Error loading diagnostic report:', err);
          ...
          onPress: async () => {
            try {
              ...
              if (res.status) {
                ...
                const response = await getpatientprofiledetail(patientId);
                setMemberDetails(response.data);
                const historyRes = await PatientHistoryData({ uhid: response.data.uhid, hiType: 1});
  ...
  if (!history || !history.length) {
    return (
      <View style={styles.centered}>
        <Text style={styles.noData}>No diagnostic reports available.</Text>
      </View>
    );
  }
      ...
      {showData && (
        <ScrollView>
          ...
          <View style={styles.headerSection}>
            <Icon name="medical-bag" size={24} color="#0A3C97" />
            <Text style={styles.header}>Diagnostic Reports</Text>
          </View>
              ...
              <View style={styles.cardContent}>
                ...
                <View style={styles.cardActions}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => handleDiscardRecord(memberDetails.uhid, item.regNo, item.docNo, 1)}
                  >
          ...
          <View style={styles.modalContainer}>
            ...
            {pdfLoading ? (
              <View style={styles.loadingContainer}>
                <ActivityIndicator size="large" color="#0A3C97" />
                <Text style={styles.loadingText}>Loading diagnostic report...</Text>
              </View>
            ) : selectedPdf ? (
              ...
              <View style={styles.errorContainer}>
                <Text style={styles.error}>Failed to load diagnostic report.</Text>
              </View>
            )}
          </View>
        </View>
...
export default GetSaveDiagnostic; 